...
export default class GridFooter extends Component {
    // constructor(props) {
    //     super(props);
    //     // console.log(this.props.currentTable.fields);
...
//     };
    ...
    render() {
        const footerItems = this.props.currentTable.fields.map((elem, index) => {
            return <TotalField total={elem.footerTotal} key={Date.now() - index}/>;
        });
        return (
            <div className="footer__wrap">
                <TotalField total={footerItems.length} index={true} key={Date.now()}/> {/* for index col */}
                {footerItems}
            </div>
        );
